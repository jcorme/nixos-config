...
let
  ...
  makeBinfmtLine = name: { recognitionType, offset, magicOrExtension
                         , mask, interpreter, preserveInterpreter
                         , preserveArgvZero, openBinary
                         , matchCredentials, fixBinary, ...
                         }: let
    type = if recognitionType == "magic" then "M" else "E";
    offset' = toString offset;
    mask' = toString mask;
    interpreter' = if preserveInterpreter then interpreter else "/run/binfmt/${name}";
          ...
          options = {
            ...
            interpreter = mkOption {
              description = ''
                ...
                Note that by default, the actual registration will point
                to /run/binfmt/''${name}, so the kernel interpreter length
                limit doesn't apply. To disable this behavior, set the
                preserveInterpreter option to true.
              '';
              type = types.path;
            };

            preserveInterpreter = mkOption {
              default = false;
              description = ''
                Whether to use the provided interpreter directly for
                registration. By default, registration will point to
                /run/binfmt/''${name}.
              '';
              type = types.bool;
            };
